...
void test_img(const char* file, float clamp) {
  ...
  from_grayscale_floats_to_ppm("clamped_decomposition.ppm", d_odata, img_w, img_h);
...
#if USE_TEXTURE_ROW
  // read data from texture
  shared[tid] = tex2D(in_tex, (float)tid, (float)y);
  shared[tid + bdim] = tex2D(in_tex, (float) tid+blockDim.x, (float)y);
#else
   // read data from global memory
